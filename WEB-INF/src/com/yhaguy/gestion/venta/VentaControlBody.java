...
public class VentaControlBody extends BodyApp {	
	...
	/************************************* AFTER SAVE **********************************/
	/**
	 * Una vez que se graba el DTO y si es de Tipo Pedido genera las reservas.
	 */
	@Override
	public void afterSave(){
		BindUtils.postGlobalCommand(null, null, "generarReservas", null);
	}
	...
	public void generarReservas() {
		...
		try {
			...
			if (this.dto.getReserva() == null) {
				MyPair tipoReserva = getDtoUtil().getReservaVenta();				
				this.crearReserva(this.dto, tipoReserva);

			} else {
				Hashtable<Long, ReservaDetalleDTO> itemsRes = new Hashtable<Long, ReservaDetalleDTO>();
				Hashtable<Long, VentaDetalleDTO> itemsVta = new Hashtable<Long, VentaDetalleDTO>();
				
				for (ReservaDetalleDTO itemRes : dto.getReserva().getDetalles()) {
					itemsRes.put(itemRes.getIdDetalleOrigen(), itemRes);
				}

				for (VentaDetalleDTO item : this.dto.getDetalles()) {					
					itemsVta.put(item.getId(), item);
					
					if(item.isServicio() == false) {
						
						if (item.isReservado() == false) {
							ReservaDetalleDTO det = new ReservaDetalleDTO();
							det.setArticulo(item.getArticulo());
							det.setCantidadReservada(item.getCantidad());
							det.setIdDetalleOrigen(item.getId());
							item.setReservado(true);

							itemsRes.put(det.getIdDetalleOrigen(), det);
							this.dto.getReserva().getDetalles().add(det);

						} else {
							ReservaDetalleDTO itemRes = itemsRes.get(item.getId());
							long dif = itemRes.getCantidadReservada() - item.getCantidad();
							itemRes.setCantidadReservada(item.getCantidad());
							itemRes.setCantidadActualizada(dif);
						}
					}					
				}
				
				for (ReservaDetalleDTO itemRes : dto.getReserva().getDetalles()) {
					if (itemsVta.get(itemRes.getIdDetalleOrigen()) == null) {
						itemRes.setCantidadActualizada(itemRes.getCantidadReservada());
						itemRes.setCantidadReservada(0);
						itemRes.setIdDetalleOrigen(Configuracion.ID_ITEM_RESERVA_LIBERADA);
					}
				}

				Object[] obj = ctr.modificarReserva(dto.getReserva(),
						ControlLogica.MODIFICAR_RESERVA);

				reservaOK = (boolean) obj[0];
				reservaMsg = (String) obj[1];

				this.dto = (VentaDTO) this.saveDTO(this.dto);
			}

			// Si algunos items no se pudieron reservar despliega un mensaje de
			// error
			if (reservaOK == false) {
				this.mensajeError(reservaMsg);
			}
